...
data Command
  ...
  | CmdRules
  | -- | save baseline
    CmdBaseline !FilePath
  | -- | show upgrade path
    CmdUpgradePath !FilePath
  | -- | path, optional port
    CmdUI !FilePath !(Maybe Int)
  deriving stock (Show)
...
parseOptions =
  info
    ...
    ( fullDesc
        ...
        <> progDesc
          "Discover workflow sprawl, detect drift, validate against policies, \
  ...
  subparser
    ( command
        ...
        <> command
          "fix"
          ( info
              (CmdFix <$> pathArg <*> switch (long "write" <> help "Actually modify files (default: dry-run)"))
        ...
        <> command
          "explain"
          ( info
              (CmdExplain . T.pack <$> strArgument (metavar "RULE_ID" <> help "Rule ID to explain"))
