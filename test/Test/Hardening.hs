...
checkoutStepWithPC = checkoutStepNoPC
...
tests :: TestTree
tests = testGroup "Hardening"
  [ testGroup "HARD-001 persist-credentials"
  ...
  , testGroup "HARD-002 default shell unset"
      [ testCase "detects run steps without shell" $ do
          ...
          assertBool "Message mentions step count"
      ...
      , testCase "counts multiple unshelled steps in one finding" $ do
          let wf = mkWf [mkJob "build" [runStepNoShell, runStepNoShell]]
              findings = evaluatePolicy hard002DefaultShellUnset wf
          length findings @?= 1
          assertBool "Message mentions count 2"
            ("2" `T.isInfixOf` findingMessage (head findings))
      ]

  , testGroup "HARD-003 pull_request_target risk"
      [ testCase "detects pull_request_target with checkout" $ do
          let wf = Workflow "Test" "test.yml"
