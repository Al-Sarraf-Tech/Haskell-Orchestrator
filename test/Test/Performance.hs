...
tests = testGroup "Performance"
  [ testGroup "PERF-001 Missing Cache"
  ...
  , testGroup "PERF-002 Sequential Parallelizable Jobs"
...
nodeInstallStep = Step Nothing (Just "Install deps") Nothing
  (Just "npm install") Map.empty Map.empty Nothing Nothing

rustBuildStep :: Step
rustBuildStep = Step Nothing (Just "Build") Nothing
  (Just "cargo build --release") Map.empty Map.empty Nothing Nothing

cacheStep :: Step
cacheStep = Step Nothing (Just "Cache") (Just "actions/cache@v3")
  Nothing Map.empty Map.empty Nothing Nothing

echoStep :: Step
echoStep = Step Nothing (Just "Echo") Nothing
  (Just "echo hello") Map.empty Map.empty Nothing Nothing
