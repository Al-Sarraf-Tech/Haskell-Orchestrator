...
tests :: TestTree
tests = testGroup "ActionsPin"
  [ testGroup "analyzePinning"
      ...
      , testCase "multiple actions across jobs are all analysed" $ do
          let step1 = mkUsesStep "actions/checkout@v4"
              ...
              wf = mkWf [ mkJob "build" [step1, step2]
                        , mkJob "test"  [step3]
                        ]
              pins = analyzePinning wf
          length pins @?= 3
  ...
  , testGroup "PinStatus ordering"
      [ testCase "AlreadyPinned < CanPin" $
          assertBool "AlreadyPinned should be less than CanPin"
            (AlreadyPinned < CanPin)

      , testCase "CanPin < LocalAction" $
          assertBool "CanPin should be less than LocalAction"
            (CanPin < LocalAction)

      , testCase "LocalAction < DockerAction" $
          assertBool "LocalAction should be less than DockerAction"
