...
import Orchestrator.Types (FindingCategory (..), RuleTag (..), Severity (..))
  ...
  testGroup
    ...
    [ testCase "No conflicts on empty pack" $ do
      ...
      testCase "PERM-001 and PERM-002 detected as Overlapping" $ do
        let conflicts = detectConflicts defaultPolicyPack
            permConflicts =
              filter
                ( \c ->
                    (ruleA c == "PERM-001" && ruleB c == "PERM-002")
                      || (ruleA c == "PERM-002" && ruleB c == "PERM-001")
                )
                conflicts
      ...
      testCase "renderConflicts non-empty contains rule IDs" $ do
        ...
        assertBool
          "Rendered text should contain the conflicting rule ID"
