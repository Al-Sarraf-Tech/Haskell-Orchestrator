...
inputWithStep :: Step
inputWithStep = Step Nothing (Just "Deploy") (Just "some/action@v1")
  Nothing
...
tests :: TestTree
tests = testGroup "ReuseRule"
  [ testGroup "REUSE-001 Reusable Workflow Input Validation"
      ...
      , testCase "detects reusable workflow with no input references" $ do
          ...
          assertBool "Should find REUSE-001 for workflow_call without inputs"
            (not (null findings))
      ...
      , testCase "no finding for empty reusable workflow" $ do
          ...
          assertBool "Empty reusable workflow triggers REUSE-001 (no inputs defined)"
            (not (null findings))
      ]

  , testGroup "REUSE-002 Reusable Workflow Unused Outputs"
      [ testCase "rule has correct ID" $
      ...
      , testCase "detects reusable workflow that sets no outputs" $ do
          ...
          assertBool "Should find REUSE-002 for workflow_call without outputs"
            (not (null findings))
      ...
      , testCase "no finding when step sets GITHUB_OUTPUT" $ do
          let wf = mkWfCall [mkJob "build" [outputStep]]
              findings = ruleCheck reuseUnusedOutputRule wf
          assertBool "Step setting GITHUB_OUTPUT should not trigger REUSE-002"
            (null findings)
      ...
      , testCase "finding message mentions workflow name" $ do
          let wf = Workflow "MyReusableWorkflow" "my-reuse.yml"
