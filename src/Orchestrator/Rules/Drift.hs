...
driftVersionRule = PolicyRule
  ...
  , ruleCheck = \wf ->
      ...
      in Map.foldrWithKey (\action vs acc ->
            let distinct = sort (nub vs)
                msg = "Action '" <> action
                      <> "' is used at multiple versions: "
                      <> T.intercalate ", " distinct
                      <> ". Pin all uses to the same version."
                finding = mkFinding' Info Drift "DRIFT-001" msg
                            (wfFileName wf)
                            Nothing
                            (Just $ "Unify all references to '" <> action
...
collectActionVersions wf =
  foldr (\(action, ver) m -> Map.insertWith (++) action [ver] m)
        Map.empty
        refs
  where
