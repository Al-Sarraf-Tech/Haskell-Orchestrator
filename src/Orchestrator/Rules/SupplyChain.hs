      ...
      ruleCheck = \wf ->
        let hasPRT = any (triggerHasName "pull_request_target") (wfTriggers wf)
         ...
         in [ mkFinding'
                ...
                Nothing
                ( Just
                    "Remove the explicit 'ref' input from actions/checkout, \
...
checkArtifactPoisoning fp isWfRun job =
  let steps = jobSteps job
      ...
      note =
        if isWfRun
          then
            " (workflow_run trigger increases severity: artifacts may \
   ...
   in [ mkFinding'
          ...
          ( "Job '"
              <> jobId job
              <> "' downloads an artifact and then executes \
          ...
          Nothing
          ( Just
              "Verify artifact integrity (e.g. cosign/sigstore) before \
      ...
      ruleCheck = \wf ->
        let hasIdTokenWrite = workflowHasIdTokenWrite wf
         ...
         in [ mkFinding'
                ...
                Nothing
                ( Just
                    "Remove 'id-token: write' or add an OIDC-consuming \
                    \deployment step."
                )
    ...
    | otherwise ->
        let slug = ownerRepo uses
         in [ mkFinding'
                ...
                ( "Action '"
                    <> slug
                    <> "' is on the abandoned-action watchlist. \
