...
data GraphEdge = GraphEdge
  { edgeFrom :: !Text  -- ^ The job that depends
  , edgeTo   :: !Text  -- ^ The job depended upon
...
data JobGraph = JobGraph
  ...
  , graphAdjacency :: !(Map Text [Text])  -- ^ job -> jobs it needs
  , graphReverse  :: !(Map Text [Text])   -- ^ job -> jobs that need it
  } deriving stock (Show)

-- | Results of graph analysis.
data GraphAnalysis = GraphAnalysis
  { gaCycles        :: ![[Text]]       -- ^ Detected cycles (each as a path)
  , gaOrphans       :: ![Text]         -- ^ Jobs with no dependents and no needs
  , gaCriticalPath  :: ![Text]         -- ^ Longest dependency chain
  , gaTopoOrder     :: !(Maybe [Text]) -- ^ Topological order (Nothing if cycles)
  , gaMaxDepth      :: !Int            -- ^ Maximum dependency depth
  } deriving stock (Show)
...
buildJobGraph wf =
  let jobs = wfJobs wf
      ...
      edges = [ GraphEdge (jobId j) n
              | j <- jobs, n <- jobNeeds j ]
      adj = foldl' (\m (GraphEdge from to) ->
              Map.insertWith (++) from [to] m
            ) (Map.fromList [(jobId j, []) | j <- jobs]) edges
      rev = foldl' (\m (GraphEdge from to) ->
...
dfsAllCycles adj nodes =
  let (cycles, _) = foldl' (\(found, visited) node ->
        if Set.member node visited
          then (found, visited)
          else let (newCycles, newVisited) = dfs adj node [] Set.empty visited
               in (found ++ newCycles, newVisited)
        ) ([], Set.empty) nodes
  in cycles

dfs :: Map Text [Text] -> Text -> [Text] -> Set Text -> Set Text
    -> ([[Text]], Set Text)
dfs adj node path inPath visited
  ...
  | otherwise =
      let neighbors = Map.findWithDefault [] node adj
          ...
          (cycles, finalVisited) = foldl' (\(cs, v) neighbor ->
...
findOrphanedJobs g
  ...
  | otherwise =
      let adj = graphAdjacency g
          ...
          isolated = [ n | n <- Set.toList (graphNodes g)
...
findCriticalPath g =
  let adj = graphAdjacency g
      ...
      roots = [ n | n <- Set.toList (graphNodes g)
...
kahn' adj inDeg (n:rest) result =
  let deps = Map.findWithDefault [] n adj
      (newQueue, newInDeg) = foldl' (\(q, deg) dep ->
  ...
  , ruleCheck = \wf ->
      ...
      in concatMap (\cyc ->
           [ Finding
               ...
               , findingMessage = "Dependency cycle detected: "
                   <> T.intercalate " → " cyc
