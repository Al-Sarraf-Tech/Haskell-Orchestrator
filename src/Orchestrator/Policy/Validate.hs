...
import Orchestrator.Policy.Conflicts (ConflictType (..), conflictType, description, detectConflicts, ruleA, ruleB)

-- | Severity of a validation issue (distinct from rule finding Severity).
data IssueSeverity
  = -- | Must be fixed; pack/rule is invalid
    IssueError
  | -- | Should be investigated; may cause surprising behaviour
    IssueWarning
  | -- | Informational; does not block usage
    IssueInfo
...
data ValidationIssue = ValidationIssue
  { issueSeverity :: !IssueSeverity,
    -- | Rule ID the issue applies to, or pack name
    issueRuleId :: !Text,
    issueMessage :: !Text
  }
  ...
  JobMissingField field
    | T.toLower field `notElem` knownJobFields ->
        [ ValidationIssue
            IssueWarning
            rid
            ( "Condition 'job_missing_field' references unknown field \""
                <> field
                <> "\". Currently supported: "
                <> T.intercalate ", " knownJobFields
                <> "."
            )
        ]
