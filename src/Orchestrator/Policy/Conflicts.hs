...
data ConflictType
  = -- | Rules with opposing severity implications in same category
    Contradictory
  | -- | Duplicate or identical-scope rules
    Redundant
  | -- | Rules that partially cover the same concern
    Overlapping
          ...
          \ra rb ->
            RuleConflict
              { conflictType = Overlapping,
                ...
                ruleB = ruleId rb,
                description =
                  "PERM-001 flags missing permissions blocks; PERM-002 flags \
          ...
          \ra rb ->
            RuleConflict
              { conflictType = Redundant,
                ...
                ruleB = ruleId rb,
                description =
                  "SEC-001 and SUPPLY-001 both detect unpinned or insecure \
          ...
          \ra rb ->
            RuleConflict
              { conflictType = Redundant,
                ...
                ruleB = ruleId rb,
                description =
                  "SEC-001 and SEC-003 both inspect action pinning / supply-chain \
  ...
  where
    ...
    describeCategConflict ra rb =
      "Rules \""
        <> ruleId ra
        <> "\" ("
        <> T.pack (show (ruleSeverity ra))
        <> ") and \""
        <> ruleId rb
        <> "\" ("
        <> T.pack (show (ruleSeverity rb))
        <> ") share category "
        <> T.pack (show (ruleCategory ra))
        <> " and tags "
        <> T.pack (show (sharedTags ra rb))
