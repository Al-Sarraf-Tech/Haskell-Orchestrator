...
lookupRequired ref =
  let actionName = T.takeWhile (/= '@') ref
   in -- Try exact match first, then prefix match for org-level entries
      -- (e.g. "github/codeql-action/analyze" matches "github/codeql-action")
      case Map.lookup actionName actionPermissionCatalog of
        Just perms -> perms
        Nothing ->
