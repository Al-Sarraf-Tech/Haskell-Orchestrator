...
checkCondition wf cond = case cond of
  ...
  StepUsesPattern pat ->
    any
      (any (maybe False (simplePatternMatch pat) . stepUses) . jobSteps)
      (wfJobs wf)
...
mkFinding sev cat rid msg fp rem' =
  Finding
    { findingSeverity = sev,
      findingCategory = cat,
      findingRuleId = rid,
            ...
            chk label perms = case perms of
              Just (PermissionsAll PermWrite) ->
                [ mkFinding
                    ...
                    fp
                    (Just "Use fine-grained permissions instead of 'write-all'.")
                ]
              _ -> []
