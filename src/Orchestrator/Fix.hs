...
data FixAction = FixAction
  { faFile :: !FilePath,
    faRuleId :: !Text,
    faDescription :: !Text,
    -- | approximate line where fix applies
    faLineHint :: !(Maybe Int),
    -- | the YAML content to add/modify
    faPatch :: !Text
  }
...
data FixConfig = FixConfig
  { -- | Actually write changes (default: False = dry-run)
    fcWrite :: !Bool,
    -- | Default timeout-minutes to add (default: 60)
    fcTimeout :: !Int
  }
  deriving stock (Eq, Show)
...
renderFixDiff actions@(fa0 : _) _content =
  T.unlines $
