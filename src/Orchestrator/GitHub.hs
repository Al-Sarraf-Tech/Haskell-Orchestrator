...
data GitHubConfig = GitHubConfig
  { -- | GitHub token (from GITHUB_TOKEN)
    ghcToken :: !(Maybe Text),
    -- | Base API URL (default: api.github.com)
    ghcApiUrl :: !Text,
    -- | Request timeout in seconds
    ghcTimeout :: !Int,
    -- | Max seconds to wait for rate limit reset
    ghcMaxWait :: !Int,
    -- | Include archived repos in org scans
    ghcIncludeArchived :: !Bool,
    -- | Include forked repos in org scans
    ghcIncludeForks :: !Bool
  }
  deriving stock (Eq, Show)

-- | Default configuration for github.com.
defaultGitHubConfig :: Maybe Text -> GitHubConfig
defaultGitHubConfig mToken =
  GitHubConfig
    { ghcToken = mToken,
...
instance FromJSON RepoEntry where
  parseJSON = withObject "RepoEntry" $ \o ->
    RepoEntry
      <$> o .: "name"
      <*> o .: "full_name"
      <*> o .:? "archived" .!= False
...
makeGitHubRequest cfg path = do
  ...
  let headers =
        [ ("User-Agent", TE.encodeUtf8 $ "haskell-orchestrator/" <> orchestratorVersion),
...
instance FromJSON DirEntry where
  parseJSON = withObject "DirEntry" $ \o ->
    DirEntry
      <$> o .: "name"
      <*> o .: "path"
      <*> o .: "type"
...
filterRepos cfg = filter keep
  where
    keep r =
      (ghcIncludeArchived cfg || not (reArchived r))
...
scanRemoteRepo pack cfg owner repo = do
  ...
  case result of
    ...
    Right files -> do
      let parsed =
            [ parseWorkflowBS (T.unpack (rwfName f)) (rwfContent f)
            | f <- files
            ]
