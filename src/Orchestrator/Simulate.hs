...
triggerMatches ctx (TriggerEvents evts) =
  any
    ( \e ->
        triggerName e == ctxEventName ctx
          && branchMatches (ctxBranch ctx) (triggerBranches e)
    )
    evts
...
simulateStep ctx s =
  let status = case stepIf s of
   ...
   in SimulatedStep
        { ssName = name,
          ssAction = stepUses s,
          ssCommand = fmap (T.take 80) (stepRun s),
          ssStatus = status
        }
...
skipJob reason j =
  SimulatedJob
    { sjJobId = jobId j,
      sjName = jobName j,
      sjRunner = renderRunner (jobRunsOn j),
      sjStatus = WillSkip reason,
      sjSteps = [],
      sjMatrixKey = Nothing,
      sjNeeds = jobNeeds j,
      sjEstMins = 0
    ...
    renderJob j =
      [ statusIcon (sjStatus j)
        ...
        ++ if isRunning (sjStatus j)
          then
            map
              (\s -> "    " <> statusIcon (ssStatus s) <> " " <> ssName s)
              (sjSteps j)
              ++ [""]
          else [""]
    statusIcon WillRun = "●"
